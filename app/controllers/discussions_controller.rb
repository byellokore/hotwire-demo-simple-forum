class DiscussionsController < ApplicationController
  ...
  def index
    @discussions = Discussion.includes(:category).all.order(updated_at: :desc)
  end

  def show
    @posts = @discussion.posts.includes(:user).all.order(:created_at)
    ...
    respond_to do |format|
      if @discussion.update(discussion_params)
        ...
        if @discussion.saved_change_to_closed?
          @discussion.broadcast_action_to(@discussion,
                                          action: :replace,
